"""Add assignee_id to cards table

This migration adds the assignee_id column to the cards table,
which allows assigning cards to specific users.
"""
...
def run_migration():
    """Run the migration to add assignee_id column to cards table"""
    ...
    with engine.connect() as connection:
        ...
        try:
            ...
            if not column_exists:
                # Añadir la columna assignee_id como clave foránea
                connection.execute(text("""
                    ALTER TABLE cards 
                    ADD COLUMN assignee_id VARCHAR(255) NULL
                """))
                
                logger.info("Migration successful: Added assignee_id column to cards table")
            else:
                logger.info("Column assignee_id already exists in cards table, skipping migration")
